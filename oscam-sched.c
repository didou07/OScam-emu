 ...
 */
api_result_t oscam_api_call(const char *query)
{
    ...
    for (int i = 0; i < job->step_count; i++) {
        switch (job->steps[i].type) {
            case STEP_API:
                ...
                if (oscam_api_call(job->steps[i].data.api_query) != API_OK) {
 ...
 */
void oscam_sched_shutdown(void)
{
